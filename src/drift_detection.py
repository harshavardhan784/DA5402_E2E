...
def _recall_signal(baseline_metrics: dict, new_metrics: dict) -> float:
    """Weighted average relative drop across Recall@1 and Recall@5."""
    r1 = _relative_drop(
        baseline_metrics.get("recall_at_1", 0.0),
        new_metrics.get("recall_at_1", 0.0),
    )
    r5 = _relative_drop(
        baseline_metrics.get("recall_at_5", 0.0),
        new_metrics.get("recall_at_5", 0.0),
    )
    return 0.5 * r1 + 0.5 * r5


def _consistency_signal(baseline_metrics: dict, new_metrics: dict) -> float:
    """Weighted average relative drop across Consistency@1 and Consistency@5."""
    c1 = _relative_drop(
        baseline_metrics.get("consistency_at_1", 0.0),
        new_metrics.get("consistency_at_1", 0.0),
    )
    c5 = _relative_drop(
        baseline_metrics.get("consistency_at_5", 0.0),
        new_metrics.get("consistency_at_5", 0.0),
    )
    return 0.5 * c1 + 0.5 * c5
...
class DriftResult:
    ...
    def report(self) -> str:
        lines = [
            ...
            f"  [Signal 2b — Consistency drop 40%]",
...
def parse_args():
    ...
    p.add_argument("--weight_embedding",   type=float, default=0.20,
                   help="Weight for embedding shift signal (default 0.20)")
    p.add_argument("--weight_recall",      type=float, default=0.4,
                   help="Weight for recall drop signal (default 0.4)")
    p.add_argument("--weight_consistency", type=float, default=0.4,
                   help="Weight for consistency drop signal (default 0.4). "
