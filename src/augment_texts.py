...
--------------------------
Rule-based text augmentation for Flipkart product image-text pairs.

Seven strategies + deduplication.
"""
...
def build_augmented_dataset(
    ...
    """
    Sample n_samples rows (optionally filtering by category_1), apply each
    augmentation, deduplicate identical (image, text) pairs by priority.

    Returns long-format DataFrame:
        original_index | image_url | method | augmented_text |
        category_2 | category_3 | selling_price
    """
    random.seed(seed)
    np.random.seed(seed)
